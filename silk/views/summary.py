...
class SummaryView(View):
    ...
    def _avg_overall_time(self, filters):
        taken__aggregate = (
            models.Request.objects.filter(*filters)
            .annotate(time_spent=Sum("time_taken"))
            .aggregate(num=Avg("time_spent"))
        )
        return taken__aggregate["num"]
    ...
    def _longest_query_by_view(self, filters):
        values_list = (
            models.Request.objects.filter(*filters)
            .values_list("view_name")
            .annotate(max=Max("time_taken"))
            .filter(max__isnull=False)
            .order_by("-max")[:5]
        )
        requests = []
        for view_name, _ in values_list:
            request = (
                models.Request.objects.filter(view_name=view_name, *filters)
                .filter(time_taken__isnull=False)
                .order_by("-time_taken")[0]
            )
    ...
    def _create_context(self, request):
        ...
        filters = [
            BaseFilter.from_dict(filter_d) for _, filter_d in raw_filters.items()
        ]
    ...
    def post(self, request):
        ...
        request.session[self.filters_key] = {
