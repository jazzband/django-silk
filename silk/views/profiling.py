...
class ProfilingView(View):
    ...
    def _get_objects(
        ...
        if order_by == "Recent":
            ...
            query_set = query_set.annotate(num_queries=Count("queries")).order_by(
                "-num_queries"
            )
        elif order_by == "Time":
            query_set = query_set.order_by("-time_taken")
        elif order_by == "Time on queries":
            query_set = query_set.annotate(db_time=Sum("queries__time_taken")).order_by(
                "-db_time"
            )
        elif order_by:
    ...
    def get(self, request, *args, **kwargs):
        return render(
            request,
            "silk/profiling.html",
            self._create_context(request, *args, **kwargs),
        )
    ...
    def post(self, request):
        ...
        request.session[self.session_key_profile_filters] = {
