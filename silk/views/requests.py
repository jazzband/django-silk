...
class RequestsView(View):
    ...
    order_by = {
        "start_time": {"label": "Recent", "additional_query_filter": None},
        "path": {"label": "Path", "additional_query_filter": None},
        "num_sql_queries": {"label": "Num. Queries", "additional_query_filter": None},
        "time_taken": {
            "label": "Time",
        ...
        "db_time": {
            ...
            "additional_query_filter": lambda x: x.annotate(
                db_time=Sum("queries__time_taken")
            ).filter(db_time__gte=0),
        },
    }
    order_dir = {"ASC": {"label": "Ascending"}, "DESC": {"label": "Descending"}}
    view_style = {"card": {"label": "Cards"}, "row": {"label": "Rows"}}
    ...
    def options_order_by(self):
        return [
            {"value": x, "label": self.order_by[x]["label"]}
            for x in self.order_by.keys()
        ]

    @property
    def options_order_dir(self):
        return [
            {"value": x, "label": self.order_dir[x]["label"]}
            for x in self.order_dir.keys()
        ]

    @property
    def options_view_style(self):
        return [
            {"value": x, "label": self.view_style[x]["label"]}
            for x in self.view_style.keys()
        ]

    def _get_paths(self):
        return (
            Request.objects.values_list("path", flat=True).order_by("path").distinct()
        )
    ...
    def _get_methods(self):
        return (
            Request.objects.values_list("method", flat=True)
            .order_by("method")
            .distinct()
        )

    def _get_objects(
        self, show=None, order_by=None, order_dir=None, path=None, filters=None
    ):
        if not filters:
            filters = []
        if not show:
        ...
        query_set = query_set.order_by(
            "{}{}".format("-" if order_dir == "DESC" else "", order_by)
        )
        if path:
    ...
    def post(self, request):
        ...
        filters = {
            ...
            **{
                k: v
                for k, v in previous_session.items()
                if k in ["show", "order_by", "order_dir", "view_style"]
            },
